...
function stmpili_funcion_crear_taxonomia() {
	...
	$labels = array(
		'name'							=> esc_html__( 'Composición', 'stmpili' ),
		'singular_name'					=> esc_html__( 'Composición', 'stmpili' ),
		'menu_name'						=> esc_html__( 'Composiciones', 'stmpili' ),
		'all_items'						=> esc_html__( 'Todas las composiciones', 'stmpili' ),
		'edit_item'						=> esc_html__( 'Editar la composición', 'stmpili' ),
		'view_item'						=> esc_html__( 'Ver la composición', 'stmpili' ),
		'update_item'					=> esc_html__( 'Actualizar la composición', 'stmpili' ),
		'add_new_item'					=> esc_html__( 'Añadir nueva composicón', 'stmpili' ),
		'new_item_name'					=> esc_html__( 'Nombre de la nueva composición', 'stmpili' ),
		'parent_item'					=> esc_html__( 'Nombre composición superior', 'stmpili' ),
		'parent_item_colon'				=> esc_html__( 'Nombre composición superior:', 'stmpili' ),
		'search_items'					=> esc_html__( 'Buscar composiciones', 'stmpili' ),
		'popular_items'					=> esc_html__( 'Composiciones más populares', 'stmpili' ),
		'separate_items_with_commas'	=> esc_html__( 'Separar las composiciones con comas', 'stmpili' ),
		'add_or_remove_items'			=> esc_html__( 'Añadir o eliminar composiciones', 'stmpili' ),
		'choose_from_most_used'			=> esc_html__( 'Elegir entre las composiciones más utilizadas', 'stmpili' ),
		'not_found'						=> esc_html__( 'Composición no encontrada', 'stmpili' )
	);
	
	$args = array(
		'label'				=> esc_html__( 'Composiciones', 'stmpili' ),
		'labels'			=> $labels,
		'public'			=> true,
		'show_ui'			=> true,
		'show_in_menu'		=> true,
	...
	<table class="form-table">
		<tr class="form-field">
			<th scope="row" valign="top">
				<label for="update"><h2>
					<?php echo esc_html__( 'Características mejoradas en STM Gallery v.2.1.', 'stmpili' ); ?>
				</h2></label>
			</th>
			<td>
				<ul>
					...
					<li><?php echo esc_html__( 'Desplazamiento entre imágenes en el lightbox mediante fade', 'stmpili' ); ?></li>
					<li><?php echo esc_html__( 'Ajuste de la altura de la última matriz', 'stmpili' ); ?></li>
					<li><?php echo esc_html__( 'Desplazamiento entre matrices mediante slider', 'stmpili' ); ?></li>
					<li><?php echo esc_html__( 'Matrización. Selección de la matriz a visualizar', 'stmpili' ); ?></li>
					<li><?php echo esc_html__( 'Posibilidad de parámetros diferentes para las imágenes destacadas', 'stmpili' ); ?></li>
				</ul>
			</td>
		</tr>
		<tr>
			<td colspan="2">
				<a href="http://sergiotoca.com/producto/stm-gallery-v-2-1/" class="button"><?php echo esc_html__( 'Actualiza ya a STM Gallery v.2.1.', 'stmpili' ); ?></a><br>
			</td>
		</tr>
	</table>
	<br>
...
function stmpili_funcion_campos_personales( $tag ) {
	...
	<tr class="form-field">
		...
		<td colspan="2">
			<a href="http://sergiotoca.com/producto/stm-gallery-v-2-1/" class="button"><?php echo esc_html__( 'Actualiza ya a STM Gallery v.2.1.', 'stmpili' ); ?></a>
		</td>
	</tr>
	<tr class="form-field">
		<th scope="row" valign="top">
	...
	<tr class="form-field">
		<th scope="row" valign="top">
			<label for="tax_taxonomia_mostrar_sidebar">
				<?php echo esc_html__( 'Mostrar barra lateral en taxonomía', 'stmpili' ); ?>
			</label>
		</th>
		<td>
			<?php
		...
		<th scope="row" valign="top">
			<label for="tax_taxonomia_mostrar_footer">
				<?php echo esc_html__( 'Mostrar pie de página en taxonomía', 'stmpili' ); ?>
			</label>
		</th>
		<td>
			<?php
	...
	<tr class="form-field">
		<th scope="row" valign="top">
			<label for="tax_taxonomia_mostrar_titulo">
				<?php echo esc_html__( 'Mostrar título de la taxonomía', 'stmpili' ); ?>
			</label>
		</th>
		<td>
			<?php
		...
		<th scope="row" valign="top">
			<label for="tax_taxonomia_mostrar_descripcion">
				<?php echo esc_html__( 'Mostrar descripción de la taxonomía', 'stmpili' ); ?>
			</label>
		</th>
		<td>
			<?php
	...
	<div class="cuadro-admin" style="width:100%;">
		<div id="div1">
			<div class="bar1"></div>
			<div class="bar2"></div>
			<div class="bar3"></div>
	...
	<p class="truco" style="display:none;"><?php echo esc_html( $tag->name ); ?></p>
...
function stmpili_funcion_guardar_datos( $term_id ) {
	...
	if ( isset( $_POST['tax_meta'] ) ) {
		...
		foreach ( $tax_keys as $key ) {
			...
			$tax_meta['destacada_igual_resto'] = ( !empty( $_POST['tax_meta']['destacada_igual_resto'] ) ) ? 'si' : 'no';
...
function stmpili_funcion_creacion_pila( $composicion ) {
	...
	foreach ( $auxiliar as $pieza ) {
		...
		$id = $pieza['id'];
		$producto_meta = get_option( "stmpili_producto_$id" );
		if ( $producto_meta['destacado_en_' . $composicion_id] == 'si' ){
		...
		for ( $y=1; $y < $stmpili_filas + 1; $y++ ) {
			for ( $x = 1; $x < $stmpili_columnas + 1; $x++ ) {
				if ( $y == 1 && $x == 1 && $tax_meta['composicion_tipo_cambio'] == 0 && $matriz != 1 ) {
					$anterior = $matriz - 1;
					$tabla_visual .= '<div style="width: ' . esc_attr( $ancho_columna ) . 'px; height: ' . esc_attr( $stmpili_filas * $ancho_columna ) . 'px; position: absolute; top: 0; left: 0;" class="cambio" onclick="document.getElementById(\'tabla-' . esc_js( $anterior ) . '\').style.display=\'block\';document.getElementById(\'tabla-'. esc_js( $matriz ) .'\').style.display=\'none\';"><span class="icono icon-left-open" style="font-size: ' . esc_attr( $ancho_columna ) . 'px;position:relative;left:-'. esc_attr( $ancho_columna/2 ) . 'px;top:' . esc_attr( $stmpili_filas * $ancho_columna / 2 ) . 'px;"></span></div>';
				}
				...
				if ($pieza == 0 ) {
					...
					if ( $pieza == -1 ) {
						...
						if ( $stmpili_pila[$pieza]['visualizada'] != true ) {
							...
							$tabla_visual .='<div class="imagen pieza-' . esc_attr( $pieza ) . '" style="width: ' . esc_attr( $ancho_marco ) . 'px; height: ' . esc_attr( $alto_marco ) . 'px; position: absolute; left: ' . esc_attr( $final_x ) . 'px; top: ' . esc_attr( $final_y ) . 'px;" onclick="stmpili_funcion_crear_lightbox(\'' . esc_url( str_replace( ' ', '\/', urldecode( $stmpili_pila[$pieza]['URL'] ) ) ) . '\',' . $ancho . ', ' . $alto . ', \'' . esc_html( $stmpili_pila[$pieza]['Title'] ) . '\', \'' .  esc_url( str_replace( ' ', '\/', urldecode( $stmpili_pila[$pieza]['Link'] ) ) ) . '\', \'' . esc_url( plugin_dir_url( __FILE__ ) ) . 'cierre.gif\', \'' . $tax_meta['img_margen_color'] . '\', ' . $tax_meta['img_margen_grosor'] . ', ' . $tax_meta['img_margen_redondeo'] . ', \'' . $tax_meta['img_borde_color'] . '\', ' . $tax_meta['img_borde_grosor'] . ', ' . $tax_meta['img_borde_redondeo'] . ');">';
				...
				if ( $y == 1 && $x == $stmpili_columnas && $tax_meta['composicion_tipo_cambio'] == 0 && $matriz != $stmpili_matriz_actual ) {
					...
					$tabla_visual .= '<div style="width: ' . esc_attr( $ancho_columna ) . 'px; height: ' . esc_attr( $stmpili_filas * $ancho_columna ) . 'px; position: absolute; top: 0; left: ' . esc_attr( $posicion_x ) . 'px;" class="cambio" onclick="document.getElementById(\'tabla-' . esc_js( $siguiente ) . '\').style.display=\'block\';document.getElementById(\'tabla-' . esc_js( $matriz ) . '\').style.display=\'none\';"><span class="icono icon-right-open" style="font-size: ' . esc_attr( $ancho_columna ) . 'px;position:relative;top:' . esc_attr( $stmpili_filas * $ancho_columna / 2 ) . 'px;"></span></div>';
				}
			}
		}
		if ( $tax_meta['composicion_tipo_cambio'] == 1 && $matriz < $stmpili_matriz_actual ) {
			$proxima = $matriz + 1;
			$tabla_visual .='<div id="transicion-' . esc_attr( $matriz ) . '" class="continuacion" style="width: ' . esc_attr( $stmpili_columnas * $ancho_columna ) . 'px; height: ' . esc_attr( $ancho_columna ) . 'px; position: absolute; left: 0; top: ' . esc_attr( $stmpili_filas * $ancho_columna ) . 'px;" colspan="24" onclick="document.getElementById(\'tabla-' . esc_js( $proxima ) . '\').style.display=\'inline-table\'; elemento = document.getElementById(\'transicion-' . esc_js( $matriz ) . '\');document.getElementById(\'fondo\').style.height=\'' . esc_js( $alto_acumulado ) . 'px\';elemento.parentNode.removeChild(elemento);"><span class="icono icon-down-open" style="font-size: ' . esc_attr( $ancho_columna ) . 'px;"></span></div>';
		}
		$tabla_visual .= '</div>';
	}
...
function stmpili_funcion_estilos_css( $composicion, $ancho ) {
	...
	switch ( $proporcion ) {
		case 0.8:
			$estilo .= '.titulo-enlace { font-size: normal; }';
			break;
		case 0.6:
...
function stmpili_crear_cuadro_pila( $termino_id ) {
		
	$matriz_elementos = array();
	
	$tax_meta = get_option( "stmpili_taxonomy_" . $termino_id );
	query_posts( array(
		'posts_per_page'	=> -1,
		'post_type'		=> 'attachment',
		'post_status'	=> 'any',
	...
	if ( have_posts() ) {
		while ( have_posts() ) {
			...
			$taxonomias_articulo = get_the_terms( $id, 'stmpili_txn' );
			$taxonomias = '';
			foreach ( $taxonomias_articulo as $taxonomia ) {
				$taxonomias .= $taxonomia->name . ', ';
			}
			$fichero = wp_get_attachment_url();
			$producto_meta = get_option("stmpili_producto_$id");
			$destacado = ( !isset( $producto_meta['destacado_en_' .$termino_id] ) ) ? 'no' : $producto_meta['destacado_en_' . $termino_id];
			$matriz_elementos[] = array(
				'id'			=> $id,
				'titulo'		=> $titulo,
				'destacado'		=> $destacado,
	...
	$matriz_sueltas = array();

	foreach ( $matriz_elementos as $elemento ) {
		$aparece = false;
		$id_elemento = $elemento['id'];
		$taxonomias_elemento = get_the_terms( $id_elemento, 'stmpili_txn');

		foreach ( $taxonomias_elemento as $taxonomia ) {
			if ( $taxonomia->term_id == $termino_id ) {
				$aparece = true;
			}
		}
		if ( $aparece == true ) {
			
			$productos_seleccionados[] = $elemento;
	...
	$resultado = '<h1>' . esc_html__( 'Elementos de la composición', 'stmpili' ) . '</h1>';
	$resultado .= '<table id="datos-elementos">';
	$resultado .= '<tr class="cabecera-tabla" style="font-weight: bold;"><td>' . esc_html__( 'Id producto', 'stmpili' ) . '</td><td>' . esc_html__( 'Título', 'stmpili' ) . '</td><td>' . esc_html__( 'Destacado', 'stmpili' ) . '</td><td>' . esc_html__( 'Eliminar', 'stmpili' ) . '</td><td>' . esc_html__( 'Composiciones', 'stmpili' ) . '</td><td>' . esc_html__( 'Imagen', 'stmpili' ) . '</td></tr>';
	foreach ( $productos_seleccionados as $elemento ) {
		$resultado .= '<tr class="' . $elemento['id'] . '">';
			$resultado .= '<td>' . $elemento['id'] . '</td>';
			$resultado .= '<td>' . $elemento['titulo'] . '</td>';
			$resultado .= '<td>';
				$checkbox = ( $elemento['destacado'] == 'si' ) ? '<input class="destacado-' . esc_attr( $elemento['id'] ) . '" type="checkbox" onchange="destaca(this.value)" value="' . esc_attr( $elemento['id'] ) . '" checked>' : '<input class="destacado-' . esc_attr( $elemento['id'] ) . '" type="checkbox" onchange="destaca(this.value)" value="' . esc_attr( $elemento['id'] ) . '">';
				$resultado .= $checkbox;
			$resultado .= '</td>';
			$resultado .= '<td><input type="button" onclick="elimina(' . esc_js( $elemento['id'] ) . ')" value="' .  esc_html__( 'Eliminar', 'stmpili' ) . '"></td>';
			$resultado .= '<td>' . esc_html( $elemento['taxonomias'] ) . '</td>';
			$resultado .= '<td><img src="' . esc_url( str_replace( ' ', '\/', urldecode( $elemento['imagen'] ) ) ) . '" height="50px"></td>';
		$resultado .= '</tr>';
		
	}
	$resultado .= '</table>';

	$resultado .= '<table id="campos-select">';
	$resultado .= '<tr>';
	$resultado .= '<td>' . esc_html__( 'Elementos actuales en la composición', 'stmpili' ) . ': <input type="text" value="' . esc_attr( count( $productos_seleccionados ) ) . '" readonly></td>';
		$resultado .= '<td>' . esc_html__( 'Productos sueltos', 'stmpili' ) . ': ';
			$resultado .= '<select id="productos-sueltos" onchange="appendSueltos(this.value);">';
				if ( count( $matriz_sueltas ) == 0 ) {
					$resultado .= '<option value="0">' . esc_html__( 'No hay más productos', 'stmpili' ) . '</option>';
				} else {
					$resultado .= '<option value="0">' . esc_html__( 'Selecciona el producto', 'stmpili' ) . '</option>';
					foreach( $matriz_sueltas as $suelta ) {
						$resultado .= '<option value="' . esc_attr( $suelta['id'] ) . '">' . esc_html( $suelta['titulo'] ) . '</option>';
					}
				}
			$resultado .= '</select>';
		$resultado .= '</td>';

	$resultado .= '<td><input type="button" onclick="todas()" value="' .  esc_html__( 'Todas las imágenes', 'stmpili' ) . '"></td>';
...
add_action( 'wp_ajax_eliminar_articulo', 'stmpili_funcion_eliminar_articulo' );
add_action( 'wp_ajax_destacar_articulo', 'stmpili_funcion_destacar_articulo' );
add_action( 'wp_ajax_incorporar_articulo', 'stmpili_funcion_incorporar_articulo' );
add_action( 'wp_ajax_incorporar_todas', 'stmpili_funcion_incorporar_todas' );

function stmpili_funcion_incorporar_articulo() {
	
	check_ajax_referer( 'devolucion_matriz' );
	
	$id = $_POST['nuevo'];
	$composicion = $_POST['truco'];
	
	wp_set_object_terms( $id, $composicion, 'stmpili_txn', true );
	
	$termino = get_term_by( 'name', $composicion, 'stmpili_txn' );
	
	$resultado = stmpili_crear_cuadro_pila( $termino->term_id );
...
function stmpili_funcion_incorporar_todas() {
	...
	if ( have_posts() ) {
		while ( have_posts() ) {
			...
			$esta_en_pila = false;
			foreach ( $taxonomias_articulo as $taxonomia ) {
				if ( $taxonomia == $composicion ) {
					$esta_en_pila = true;
				}
			}
			
			if ( $esta_en_pila == false ) {
				wp_set_object_terms( $id, $composicion, 'stmpili_txn', true );
			}
		}
	}
	
	$termino = get_term_by( 'slug', $composicion, 'stmpili_txn' );
...
function stmpili_funcion_eliminar_articulo() {
	...
	wp_remove_object_terms( $id, $composicion, 'stmpili_txn' );
...
function stmpili_funcion_destacar_articulo() {
	...
	$tax_meta = get_option("stmpili_producto_$id");
	$termino = get_term_by('name', $composicion, 'stmpili_txn' );
	$termino_id = $termino->term_id;
	$destacado = ( !isset( $tax_meta['destacado_en_' . $termino_id] ) ) ? 'si' : $tax_meta['destacado_en_' . $termino_id];

	$tax_meta['destacado_en_' . $termino_id] = ( $tax_meta['destacado_en_' . $termino_id] == 'no') ? 'si' : 'no';

	update_option( "stmpili_producto_$id", $tax_meta );
...
add_action( 'wp_ajax_mostrar_pila', 'stmpili_funcion_mostrar_pila' );

function stmpili_funcion_mostrar_pila() {
	...
	$resultado = stmpili_crear_cuadro_pila( (int) $termino->term_id );
...
function stmpili_sanitize_validate_input ( $key, $value ) {
	...
	switch ( $key ) {
		...
		case 'destacada_margen_grosor':
		case 'destacada_margen_redondeo':
		case 'destacada_borde_grosor':
		case 'destacada_borde_redondeo':
		case 'destacada_sombra_grosor':
		case 'composicion_tipo_cambio':
		case 'img_separacion':
		case 'ajuste_final':
			$correcto = intval ( $value );
			break;
		case 'img_margen_color':
		case 'img_borde_color':
		case 'img_sombra_color':
		case 'destacada_margen_color':
		case 'destacada_borde_color':
		case 'destacada_sombra_color':
		case 'composicion_fondo_color':
			$is_hexa_color = strpos ( $value, '#' );
			if ( $is_hexa_color === false ) {
				if ( $key == 'img_margen_color' || $key == 'destacada_margen_color' ) {
					$correcto = '#fff';
				} else {
					$correcto = '#000';
				}
